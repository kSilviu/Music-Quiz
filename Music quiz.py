...
def sanitize_filename(name):
    return re.sub(r'\W+', '', name.lower()) + '.txt'

def add_song():
    new_artist = input("Enter the artist's name: ").strip()

    #checks if the artist is already in the database
    with open('artist.txt', 'r') as f:
        existing_artists = [line.strip() for line in f]

    if new_artist not in existing_artists:
        with open('artist.txt', 'a+') as f:
            f.write(new_artist + '\n')
        print(f"Artist {new_artist} added.")
    else:
        print(f"Artist {new_artist} already exists. You can add more songs.")
    ...
    filename = sanitize_filename(new_artist)
    with open(filename, 'a') as f:
        for song in songs:
            f.write(song + '\n')

    print(f"{new_artist}'s songs have been updated.")
...
while play_amount == 1:
    ...
    artist_random = random.choice(artist_list)
    filename = sanitize_filename(artist_random)

    if os.path.exists(filename):
        with open(filename, "r") as f:
            song_list = [line.strip() for line in f if line.strip()]
            if not song_list:
                print(f"No songs found for {artist_random}. Skipping...")
                continue
    ...
    # removes any previous scores with the same name
    scores = [(name, val) for name, val in scores if name != username]
